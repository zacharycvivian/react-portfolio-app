...
const CyberWordle: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/CyberWordList.txt")
      ...
      .then((text) => {
        const words: WordListType = text.split("\n").map((word) => word.trim()).filter((word) => word.length === 5);
        setWordList(words);
  ...
  const handleSubmitGuess = (): void => {
    ...
    if (guess.length !== 5 || gameOver || /[^a-zA-Z]/.test(guess)) {
      alert("Each box must contain a single alphabetic letter. No spaces or special characters allowed.");
      return;
    }
    ...
    } else if (newGuesses.length >= 6) { // Assuming 6 attempts are allowed
      setTimeout(() => {
...
const renderGuessFeedback = (guess: string, isCorrectWord: boolean = false): JSX.Element[] => {
    return guess.split("").map((letter, index) => {
      const letterStyle = isCorrectWord ? styles.red : getLetterStyle(letter, index);
      return (
  ...
  return (
    <div className={styles.container}>
      ...
      <h1 className={styles.title}>Cyber Wordle</h1>
        ...
        <button className={styles.submitButton} onClick={handleSubmitGuess} disabled={gameOver}>
          Submit Guess
        </button>
