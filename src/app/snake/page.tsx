...
const SnakeGame: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('orientationchange', handleLoadOrOrientationChange);
    };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case "ArrowUp": if (dir.y === 0) setDir({ x: 0, y: -20 }); break;
        case "ArrowDown": if (dir.y === 0) setDir({ x: 0, y: 20 }); break;
        case "ArrowLeft": if (dir.x === 0) setDir({ x: -20, y: 0 }); break;
        case "ArrowRight": if (dir.x === 0) setDir({ x: 20, y: 0 }); break;
  ...
  useEffect(() => {
    ...
    const moveSnake = () => {
      ...
      if (newHead.x >= gameSize.width || newHead.x < 0 || newHead.y >= gameSize.height || newHead.y < 0 ||
  ...
  const spawnApple = () => {
    ...
    do {
      ...
      isOccupied = snake.some(segment => segment.x === potentialApple.x && segment.y === potentialApple.y);
